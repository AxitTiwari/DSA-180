...
class NewSolution {
public:
    bool f(vector<int>& arr, int i, int sum1, int sum2) {
        ...
        for (int j = i; j < arr.size(); j++) {
            include = f(arr, i+1, sum1+arr[i], sum2-arr[i]);
            exclude = f(arr, i+1, sum1, sum2);
        }

        return include || exclude;
    }

    bool memoization(vector<int>& arr, int i, int sum1, int sum2, 
        map<pair<int, pair<int, int>>, bool>& dp
    ) {

        auto key = make_pair(i, make_pair(sum1, sum2));;
        ...
        bool exclude = false;
        for (int j = i; j < arr.size(); j++) {
            include = memoization(arr, i+1, sum1+arr[i], sum2-arr[i], dp);
            exclude = memoization(arr, i+1, sum1, sum2, dp);
        }
...
public:
    bool canPartition(vector<int>& arr) {
        ...
        // ----------MY LOGIC (Day-2) (question solving)-------------
        // return s.f(arr, 0, 0, total);

        map<pair<int, pair<int, int>>, bool> dp;

        return s.memoization(arr, 0, 0, total, dp);
    }
};